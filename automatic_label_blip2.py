...
def load_blip_clip(device):
    ...
    # )
    blip_vis_processors = Blip2Processor.from_pretrained("Salesforce/blip2-flan-t5-xxl")
    blip_model = Blip2ForConditionalGeneration.from_pretrained(
        "Salesforce/blip2-flan-t5-xxl", load_in_8bit=True, device_map='auto', torch_dtype=torch.float16
    )
    return blip_model, blip_vis_processors

def forward_blipv2(raw_image, category_name, bbox, blip_model, blip_vis_processors):
    ...
    if area >= 32*32:
        ...
        #     "image": raw_image_cropped,
        #     "prompt": 'Question: what is in the image?'
        #     })
        inputs = blip_vis_processors(images=raw_image_cropped, return_tensors="pt").to(device, torch.float16)

        generated_ids = blip_model.generate(**inputs)
        captions = blip_vis_processors.batch_decode(generated_ids, skip_special_tokens=True)
        print(captions)
...
def save_mask_data(output_dir, mask_list, box_list, label_list, file_name, image_pil, output, blip_model, blip_vis_processor):
    ...
    for label, box in zip(label_list, box_list):
        ...
        instance_caption = forward_blipv2(RGB_image, name, x1y1x2y2, blip_model, blip_vis_processor)
...
def main(args):
    ...
    blip_model, blip_vis_processor = load_blip_clip(device1)
    ...
    for image_path, image_caption in zip(image_paths[start_idx:end_idx], image_captions[start_idx:end_idx]):
        ...
        # save mask data
        save_mask_data(output_dir, None, boxes_filt, pred_phrases, file_name, image_pil, output, blip_model, blip_vis_processor)
